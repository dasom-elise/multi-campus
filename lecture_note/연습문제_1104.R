...
chevrolet = df %>%
  ...
  summarise(chevrolet_hwy = mean(hwy))

ford = df %>%
  filter(manufacturer == "ford") %>%
  summarise(ford_hwy = mean(hwy))

honda = df %>%
  filter(manufacturer == "honda") %>%
  summarise(honda_hwy = mean(hwy))
...
df %>%
  filter(class == "suv") %>%
  mutate(avg_mil = (cty+hwy)/2) %>%
  group_by(manufacturer) %>%
  summarise(avg_suv = mean(avg_mil)) %>%
  arrange(desc(avg_suv)) %>%
  head(5)
